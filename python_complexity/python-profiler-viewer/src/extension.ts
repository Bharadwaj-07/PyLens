...
export function activate(context: vscode.ExtensionContext) {
    ...
    let disposable = vscode.commands.registerCommand('python-profiler-viewer.showProfilerData', async () => {
        ...
        const realtimePanel = vscode.window.createWebviewPanel(
            'profilerRealtimeView',
            'Profiler - Realtime Monitor',
        ...
        let realtimeWatcher: fs.FSWatcher;
        try {
        ...
        vscode.window.withProgress({
            ...
            return new Promise<void>((resolve, reject) => {
                const process = spawn('python', profilerArgs, {
                    cwd: directory
                });
        ...
        realtimePanel.onDidDispose(() => {
            if (realtimeWatcher) realtimeWatcher.close();
        });
    });
...
function showFinalStatsView(data: any) {
    const panel = vscode.window.createWebviewPanel(
        'profilerStatsView',
        'Python Profiler - Final Statistics',
        vscode.ViewColumn.One,
        { enableScripts: true }
    );
...
function getWebviewContent(data: any) {
    ...
    let avgCpuUsages: number[] = [];
    let avgMemUsages: number[] = [];
